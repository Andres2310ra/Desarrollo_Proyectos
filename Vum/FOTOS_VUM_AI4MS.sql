    ...
    RUTA AS ( 
     ...
     MAR_CAT AS (
        ...
        FROM
            ...
            INNER JOIN SIM_CATEGORIA AS CAT ON CAT.CAT_CODIGO=PRA.CAT_CODIGO
    WHERE
        MAR.MAR_PROPIA=1 AND MAR.ESTADO=1)
SELECT
    FOT.RUT_CODIGO,
    RUT.PDV_CODIGO,
    PVC.PVC_NOMBRE_PDV,
...
FROM 
    SIM_FOTO AS FOT 
    INNER JOIN RUTA AS RUT ON RUT.RUT_CODIGO=FOT.RUT_CODIGO
    INNER JOIN SIM_TIPO AS T1 ON T1.TIP_CODIGO=FOT.TIP_CODIGO
    INNER JOIN MARCAS AS MAR ON MAR.FOT_CODIGO=FOT.FOT_CODIGO
    INNER JOIN MAR_CAT AS CAT ON CAT.MAR_CODIGO=MAR.MAR_CODIGO
    INNER JOIN SIM_PUNTO_VENTA_CLIENTE AS PVC ON PVC.PDV_CODIGO=RUT.PDV_CODIGO AND PVC.ACT_CODIGO=RUT.ACT_CODIGO
WHERE RUT.CLI_CODIGO = 138 AND FOT.ESTADO = 1 AND CONVERT(DATE,FOT.FECHA_CREACION)>='2024-02-01'
GROUP BY 
FOT.RUT_CODIGO,RUT.PDV_CODIGO,PVC.PVC_NOMBRE_PDV,RUT.USU_LOGIN,FOT.FOT_CODIGO,FOT.FOT_RUTA,FOT.TIP_CODIGO,T1.TIP_NOMBRE,
RUT.CLI_NOMBRE,RUT.ACT_NOMBRE,FOT.FECHA_CREACION,FOT.FECHA_MODIFICACION
UNION
/*CATEGORIAS ENCUESTA*/
SELECT
    FOT.RUT_CODIGO,
...
GROUP BY 
FOT.RUT_CODIGO,RUT.PDV_CODIGO,PVC.PVC_NOMBRE_PDV,RUT.USU_LOGIN,FOT.FOT_CODIGO,FOT.FOT_RUTA,FOT.TIP_CODIGO,T1.TIP_NOMBRE,RUT.CLI_NOMBRE,RUT.ACT_NOMBRE,FOA.ENA_CODIGO,ENC.ENC_NOMBRE,FOT.FECHA_CREACION,FOT.FECHA_MODIFICACION
