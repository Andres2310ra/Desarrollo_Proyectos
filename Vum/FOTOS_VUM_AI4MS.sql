WITH CATEGORIAS AS (
    ...
    FROM 
            ...
            INNER JOIN SIM_PRODUCTO_ACTIVIDAD AS PRA ON ACT.ACT_CODIGO=PRA.ACT_CODIGO
    ...
    WHERE 
        ACT.ESTADO=1 AND MAR.MAR_PROPIA = 1 AND MAR.ESTADO = 1 AND PRA.ESTADO = 1 AND CAT.ESTADO = 1)
SELECT
    FOT.RUT_CODIGO,
    FOT.FOT_CODIGO,
    FOT.FOT_RUTA,
...
FROM 
    ...
    INNER JOIN ( 
        ...
        FROM
            SIM_CLIENTE AS CLI
            INNER JOIN SIM_ACTIVIDAD AS ACT ON CLI.CLI_CODIGO = ACT.CLI_CODIGO
            INNER JOIN SIM_USUARIO_ACTIVIIDAD AS USA ON ACT.ACT_CODIGO = USA.ACT_CODIGO
            INNER JOIN SIM_RUTA AS RUT ON USA.USA_CODIGO = RUT.USA_CODIGO
        WHERE
            CLI.ESTADO=1 AND ACT.ESTADO=1 AND RUT.ESTADO=1) AS RUT ON RUT.RUT_CODIGO=FOT.RUT_CODIGO
