...
WHERE 
CONVERT(DATE,RUT_FECHA_RUTA) >= @FECHA_RUT AND R.ESTADO>0) AS RUT 

FULL JOIN 

(SELECT DISTINCT RUT_CODIGO AS 'SIM_ACTIVIDAD_COMERCIAL' 
FROM SIM_ACTIVIDAD_COMERCIAL 
WHERE FECHA_CREACION >= @FECHA_RUT AND ESTADO>0) AS ACO ON (RUT.RUT_CODIGO=ACO.SIM_ACTIVIDAD_COMERCIAL) 

FULL JOIN 

(SELECT DISTINCT RUT_CODIGO AS 'SIM_AGOTADOS' 
FROM SIM_AGOTADOS 
WHERE FECHA_CREACION >= @FECHA_RUT AND ESTADO>0) AS AGO ON (RUT.RUT_CODIGO=AGO.SIM_AGOTADOS) 

FULL JOIN 

(SELECT DISTINCT RUT_CODIGO AS 'SIM_AUDITORIA' 
FROM SIM_AUDITORIA 
WHERE FECHA_CREACION >= @FECHA_RUT AND ESTADO>0) AS AUD ON (RUT.RUT_CODIGO=AUD.SIM_AUDITORIA) 

FULL JOIN 

(SELECT DISTINCT RUT_CODIGO AS 'SIM_CAMBIO_MARCA' 
FROM SIM_CAMBIO_MARCA 
WHERE FECHA_CREACION >= @FECHA_RUT AND ESTADO>0) AS CMA ON (RUT.RUT_CODIGO=CMA.SIM_CAMBIO_MARCA) 

FULL JOIN 

(SELECT DISTINCT RUT_CODIGO AS 'SIM_CARAS ' 
FROM SIM_CARAS 
WHERE FECHA_CREACION >= @FECHA_RUT AND ESTADO>0) AS CAR ON (RUT.RUT_CODIGO=CAR.SIM_CARAS) 

FULL JOIN 

(SELECT DISTINCT RUT_CODIGO AS 'SIM_CHEQUEO_PRECIO' 
FROM SIM_CHEQUEO_PRECIO 
WHERE FECHA_CREACION >= @FECHA_RUT AND ESTADO>0) AS CPR ON 
...
FROM SIM_EXHIBICION 
WHERE FECHA_CREACION >= @FECHA_RUT AND ESTADO>0) AS EXI ON (RUT.RUT_CODIGO=EXI.SIM_EXHIBICION) 

FULL JOIN 

(SELECT DISTINCT RUT_CODIGO AS 'SIM_FOTO' 
FROM SIM_FOTO 
WHERE FECHA_CREACION >= @FECHA_RUT AND ESTADO>0) AS FOT ON (RUT.RUT_CODIGO=FOT.SIM_FOTO) 

FULL JOIN 

(SELECT DISTINCT RUT_CODIGO AS 'SIM_INVENTARIO' 
FROM SIM_INVENTARIO 
WHERE FECHA_CREACION >= @FECHA_RUT AND ESTADO>0) AS INV ON (RUT.RUT_CODIGO=INV.SIM_INVENTARIO) 

FULL JOIN 

(SELECT DISTINCT RUT_CODIGO AS 'SIM_NOTA' 
FROM SIM_NOTA 
WHERE FECHA_CREACION >= @FECHA_RUT AND ESTADO>0) AS NOA ON (RUT.RUT_CODIGO=NOA.SIM_NOTA) 

FULL JOIN 

(SELECT DISTINCT RUT_CODIGO AS 'SIM_RESPUESTA_USUARIO' 
FROM SIM_RESPUESTA_USUARIO 
WHERE FECHA_CREACION >= @FECHA_RUT AND ESTADO>0) AS REU ON ( RUT.RUT_CODIGO=REU.SIM_RESPUESTA_USUARIO) 

FULL JOIN 

(SELECT DISTINCT RUT_CODIGO AS 'SIM_RUTERO_VALORIZADO' 
FROM SIM_RUTERO_VALORIZADO 
WHERE FECHA_CREACION >= @FECHA_RUT AND ESTADO>0) AS REV ON (RUT.RUT_CODIGO=REV.SIM_RUTERO_VALORIZADO) 

FULL JOIN 

(SELECT DISTINCT RUT_CODIGO AS 'SIM_SEGUIMIENTO_VENCIMIENTO' 
FROM SIM_SEGUIMIENTO_VENCIMIENTO WHERE FECHA_CREACION >= @FECHA_RUT AND ESTADO>0) AS SEV ON (RUT.RUT_CODIGO=SEV.SIM_SEGUIMIENTO_VENCIMIENTO) 

FULL JOIN 

(SELECT DISTINCT RUT_CODIGO AS 'SIM_TRANSFERENCIA' 
FROM SIM_TRANSFERENCIA 
WHERE FECHA_CREACION >= @FECHA_RUT AND ESTADO>0) AS TRA ON (RUT.RUT_CODIGO=TRA.SIM_TRANSFERENCIA) 

FULL JOIN 

(SELECT DISTINCT RUT_CODIGO AS 'SIM_VENCIMIENTO' 
FROM SIM_VENCIMIENTO 
WHERE FECHA_CREACION >= @FECHA_RUT AND ESTADO>0) AS VEC ON (RUT.RUT_CODIGO=VEC.SIM_VENCIMIENTO) 

FULL JOIN 

(SELECT DISTINCT RUT_CODIGO AS 'SIM_VENTAS ' 
FROM SIM_VENTAS 
WHERE FECHA_CREACION >= @FECHA_RUT AND ESTADO>0) AS VET ON (RUT.RUT_CODIGO=VET.SIM_VENTAS) 

FULL JOIN 

(SELECT DISTINCT RUT_CODIGO AS 'SIM_RECORDATORIO' 
FROM SIM_RECORDATORIO 
WHERE FECHA_CREACION >= @FECHA_RUT AND ESTADO>0) AS REC ON (RUT.RUT_CODIGO=REC.SIM_RECORDATORIO) 

FULL JOIN 

(SELECT DISTINCT RUT_CODIGO AS 'SIM_SEGUIMIENTO_RECORDATORIO' 
FROM SIM_SEGUIMIENTO_RECORDATORIO 
WHERE FECHA_CREACION >= @FECHA_RUT AND ESTADO>0) AS SER ON (RUT.RUT_CODIGO=SER.SIM_SEGUIMIENTO_RECORDATORIO) 

FULL JOIN 

(SELECT DISTINCT RUT_CODIGO AS 'SIM_CITA' 
FROM SIM_CITA 
WHERE FECHA_CREACION >= @FECHA_RUT AND ESTADO>0) AS CIT ON (RUT.RUT_CODIGO=CIT.SIM_CITA) 

FULL JOIN 

(SELECT DISTINCT RUT_CODIGO AS 'SIM_CAPACITACIONES' 
FROM SIM_CAPACITACIONES 
WHERE FECHA_CREACION >= @FECHA_RUT AND ESTADO>0) AS CAP ON (RUT.RUT_CODIGO=CAP.SIM_CAPACITACIONES) 

FULL JOIN 

(SELECT DISTINCT RUT_CODIGO AS 'SIM_GESTION_EXHIBICION' 
FROM SIM_GESTION_EXHIBICION 
WHERE FECHA_CREACION >= @FECHA_RUT AND ESTADO>0) AS GEE ON (RUT.RUT_CODIGO=GEE.SIM_GESTION_EXHIBICION) 

FULL JOIN 

(SELECT DISTINCT RUT_CODIGO AS 'SIM_ENCABEZADO_MOVIMIENTO' 
FROM SIM_ENCABEZADO_MOVIMIENTO 
WHERE FECHA_CREACION >= @FECHA_RUT AND ESTADO>0) AS EMO ON (RUT.RUT_CODIGO=EMO.SIM_ENCABEZADO_MOVIMIENTO) 

FULL JOIN 

(SELECT DISTINCT RUT_CODIGO AS 'SIM_CHEQUEO_PRECIO_ESPECIAL' 
FROM SIM_CHEQUEO_PRECIO_ESPECIAL 
WHERE FECHA_CREACION >= @FECHA_RUT AND ESTADO>0) AS CHP ON (RUT.RUT_CODIGO=CHP.SIM_CHEQUEO_PRECIO_ESPECIAL)
