SELECT
    ...
    CONVERT(INT, value) AS TIP_CODIGO,
    T5.TIP_NOMBRE,
    --T5.TIP_NOMBRE AS NOVEDAD,
...
FROM
    ...
    CROSS APPLY STRING_SPLIT(TSS.TIP_CODIGO, ',') -- Aquí dividimos los valores
    ...
    INNER JOIN (
        ...
        WHERE    
            TSS.ESTADO = 1 AND TSS.ACT_CODIGO = 392
    ) AS TTS ON TSS.TSS_CODIGO = TTS.TSS_CODIGO AND RN = 1
    ...
    INNER JOIN SIM_TIPO AS T5 ON T5.TIP_CODIGO = CONVERT(INT, value) -- Corrección aquí
WHERE 
    TSS.ESTADO = 1 AND TSS.ACT_CODIGO = 392 AND CONVERT(DATE, TSS.FECHA_CREACION) >= '2024-08-01';
